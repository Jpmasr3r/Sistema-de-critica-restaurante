...
async function getInfs() {
	try {
		let data = await fetch("../api/getSelectedUser.php").then(res => res.json());
...
async function getCriticas() {
	try {
		let data = await fetch("../api/getAllCriticasUser.php").then(res => res.json());
		console.log(data);
		return data;
	} catch (error) {
		console.log(error);
...
async function printCriticas() {
	...
	allCriticas.forEach(e => {
		...
		divCritica.addEventListener("click", async () => {
			try {
				let data = await fetch(`../api/setSelectedRestaurante.php?id=${e.restauranteID}`).then(res => res.text());
				if (data == "OK") {
